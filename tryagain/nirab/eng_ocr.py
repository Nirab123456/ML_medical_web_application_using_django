        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if existing_record:
                    ...
                    if file_path:
                        print(file_path)
                    return render(request, 'OCR.html', {'form': form, 'image_url': existing_record.image.url, 'text': text, 'file_path': file_path})
                else:
                    ...
                    if file_path:
                        print(file_path)
                    return render(request, 'OCR.html', {'form': form, 'image_url': record_image.image.url, 'text': text, 'file_path': file_path})
        else:
            form = OCRImageForm()
